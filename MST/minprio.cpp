...
struct handle {
  ...
  handle* enqueue(void* item){
      if(currentSize == MAXSIZE){
          ...
          currentSize++;
          handle[currentSize] = new handle;
          handle[currentSize]->pos = currentSize;
          handle[currentSize]->content = item;
          int t = currentSize;
          while(true){
              int compare = compFunc(handle[t/2]->content, handle[t]->content);
              if(compare < 0){
                  //Parent less than child
                  handle *tempNode = new handle;
                  tempNode->content = handle[t]->content;
                  
                  handle[t]->content = handle[t/2]->content;
                  handle[t/2]->content = tempNode->content;
                  t = t/2;
                  delete tempNode;
              }else{
                  return handle[t];
